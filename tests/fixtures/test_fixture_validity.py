...
class TestDirectedNetworkFixtures:
    ...
    @pytest.mark.parametrize("network_name,metadata", list(dn.NETWORK_METADATA.items()))
    ...
    def test_network_properties_match_metadata(self, network_name: str, metadata: dict) -> None:
        ...
        assert d_is_tree(network) == metadata["is_tree"], f"{network_name}: is_tree mismatch"
        assert d_is_binary(network) == metadata["is_binary"], f"{network_name}: is_binary mismatch"
        assert (
            d_has_parallel_edges(network) == metadata["has_parallel_edges"]
        ), f"{network_name}: has_parallel_edges mismatch"

        # For non-empty networks, test level-related properties
        if metadata["nodes"] > 1:
            assert (
                d_level(network) == metadata["level"]
            ), f"{network_name}: level mismatch (expected {metadata['level']}, got {d_level(network)})"
            assert (
                d_vertex_level(network) == metadata["vertex_level"]
            ), f"{network_name}: vertex_level mismatch"
            assert (
                d_reticulation_number(network) == metadata["reticulation_number"]
    ...
    def test_can_filter_by_category(self) -> None:
        ...
        assert len(trees) > 0
        assert all(
            dn.NETWORK_METADATA[name]["category"] == "trees"
...
class TestSemiDirectedNetworkFixtures:
    ...
    @pytest.mark.parametrize("network_name,metadata", list(sdn.NETWORK_METADATA.items()))
    def test_network_is_valid(self, network_name: str, metadata: dict) -> None:
        """
        Test that network can be created and is valid.
    ...
    def test_network_properties_match_metadata(self, network_name: str, metadata: dict) -> None:
        ...
        # Test classification properties
        assert sd_is_tree(network) == metadata["is_tree"], f"{network_name}: is_tree mismatch"
        assert sd_is_binary(network) == metadata["is_binary"], f"{network_name}: is_binary mismatch"
        assert (
            sd_has_parallel_edges(network) == metadata["has_parallel_edges"]
        ), f"{network_name}: has_parallel_edges mismatch"

        # For non-empty networks, test level-related properties
        if metadata["nodes"] > 1:
            assert (
                sd_level(network) == metadata["level"]
            ), f"{network_name}: level mismatch (expected {metadata['level']}, got {sd_level(network)})"
            assert (
                sd_vertex_level(network) == metadata["vertex_level"]
            ), f"{network_name}: vertex_level mismatch"
            assert (
                sd_reticulation_number(network) == metadata["reticulation_number"]
            ), f"{network_name}: reticulation_number mismatch"
    ...
    def test_can_filter_by_category(self) -> None:
        ...
        assert len(trees) > 0
        assert all(
            sdn.NETWORK_METADATA[name]["category"] == "trees"
...
class TestFixtureConsistency:
    ...
    def test_both_modules_have_similar_coverage(self) -> None:
        ...
        assert (
            abs(d_count - sd_count) / max(d_count, sd_count) < 0.3
