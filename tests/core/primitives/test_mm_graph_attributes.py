...
class TestMixedMultiGraphAttributes:
    ...
    def test_set_graph_attribute(self):
        ...
        assert graph._directed.graph.get("probability") == 0.5
        assert graph._undirected.graph.get("probability") == 0.5
        assert graph._combined.graph.get("probability") == 0.5

        # Test updating the attribute
        graph.set_graph_attribute("probability", 0.8)
        assert graph._directed.graph.get("probability") == 0.8
        assert graph._undirected.graph.get("probability") == 0.8
    ...
    def test_warn_on_keyword_attribute(self):
        ...
        with pytest.warns(
            UserWarning, match="Graph attribute key .* is a Python keyword"
        ) as record:
            graph = MixedMultiGraph(
                undirected_edges=[(1, 2)], attributes={"for": "value", "if": "other"}
            )
