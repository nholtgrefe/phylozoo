...
class TestSingleEdgeNetwork:
    ...
    def test_single_undirected_edge_network(self) -> None:
        ...
        with expect_mixed_network_warning():
            net = MixedPhyNetwork(undirected_edges=[(1, 2)], nodes=[(2, {"label": "A"})])
...
class TestLargeNetworks:
    ...
    def test_network_many_hybrids(self) -> None:
        ...
        nodes.append((5000, {"label": "TaxonCentral1"}))
        nodes.append((5001, {"label": "TaxonCentral2"}))
...
class TestHighDegreeNodes:
    ...
    def test_node_degree_10(self) -> None:
        ...
        nodes = [(i, {"label": f"Taxon{i}"}) for i in range(1, 10)]  # Nodes 1-9
        nodes.append((11, {"label": "Taxon10"}))  # Node 11 is the 10th leaf
        with expect_mixed_network_warning():
            net = MixedPhyNetwork(undirected_edges=edges, nodes=nodes)
...
class TestManyParallelEdges:
    ...
    def test_10_parallel_undirected_edges(self) -> None:
        ...
        nodes = [(100, {"label": "Taxon0"}), (101, {"label": "Taxon1"}), (102, {"label": "Taxon2"})]
        nodes.extend(
            [(200, {"label": "Taxon3"}), (201, {"label": "Taxon4"}), (202, {"label": "Taxon5"})]
        )
        with expect_mixed_network_warning():
            net = MixedPhyNetwork(undirected_edges=edges, nodes=nodes)
...
class TestDeepTrees:
    ...
    def test_very_deep_tree(self) -> None:
        ...
        def add_subtree(parent, depth, max_depth):
            ...
            if depth >= max_depth:
                ...
                for _ in range(3):
                    ...
                    nodes.append((node_id, {"label": f"Taxon{node_id}"}))
                    node_id += 1
                return
        ...
        nodes.append((child3, {"label": f"Taxon{child3}"}))
...
class TestComplexTopologies:
    ...
    def test_multiple_independent_hybrids(self) -> None:
        ...
        directed_edges = [
            (5, 4),
            (6, 4),  # First hybrid event (corrected: 4 is hybrid)
            (15, 14),
            (16, 14),  # Second hybrid event (corrected: 14 is hybrid)
        ]
        undirected_edges = [
            (4, 1),
            (5, 8),
            (5, 10),
            (6, 9),
            (6, 11),  # First hybrid region
            (14, 2),
            (15, 18),
            (15, 20),
            (15, 22),
            (16, 19),
            (16, 21),  # Second hybrid region (15 needs degree >= 3)
            (10, 20),
            (10, 23),
            (20, 24),  # Connect the two regions, nodes 10 and 20 need degree >= 3
        ]
        nodes = [
            (1, {"label": "A"}),
            (2, {"label": "B"}),
    ...
    def test_nested_hybridization(self) -> None:
        ...
        directed_edges = [(7, 5), (8, 5), (5, 4), (6, 4)]  # Hybrid 5  # Hybrid 4 (nested)
        undirected_edges = [
            (4, 1),
            (6, 11),
            (6, 12),
...
class TestInvalidStructures:
    ...
    def test_disconnected_network(self) -> None:
        ...
        with pytest.raises(ValueError, match="not connected"):
            MixedPhyNetwork(
                undirected_edges=[(1, 2), (3, 4)], nodes=[(2, {"label": "A"}), (4, {"label": "B"})]
            )
    ...
    def test_internal_node_degree_two(self) -> None:
        ...
        with pytest.raises(ValueError, match="degree"):
            MixedPhyNetwork(
                undirected_edges=[(1, 2), (2, 3)], nodes=[(1, {"label": "A"}), (3, {"label": "B"})]
            )
