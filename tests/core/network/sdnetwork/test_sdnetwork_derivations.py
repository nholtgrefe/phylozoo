...
from phylozoo.core.network.sdnetwork.derivations import (
    _switchings,
    displayed_trees,
    displayed_splits,
    displayed_quartets,
...
class TestSubnetwork:
    ...
    def test_with_both_parameters(self) -> None:
        ...
        subnet = subnetwork(net, ["A", "B"], suppress_2_blobs=True, identify_parallel_edges=True)
        assert isinstance(subnet, SemiDirectedPhyNetwork)
        assert sorted(subnet.taxa) == ["A", "B"]
        subnet.validate()  # Should be valid
...
class TestKTaxonSubnetworks:
    ...
    def test_with_both_parameters(self) -> None:
        ...
        subnetworks = list(
            k_taxon_subnetworks(net, k=2, suppress_2_blobs=True, identify_parallel_edges=True)
        )
...
class TestSwitchings:
    ...
    def test_single_hybrid_three_parents(self) -> None:
        ...
        net = SemiDirectedPhyNetwork(
            ...
            undirected_edges=[
                ...
                (5, 6),
                (6, 7),  # Connect parents to ensure single source component
                (5, 8),
                (5, 9),  # Tree node 5
                (6, 10),
                (6, 11),  # Tree node 6
    ...
    def test_probability_without_gamma_values(self) -> None:
        ...
        net = SemiDirectedPhyNetwork(
            directed_edges=[(5, 4), (6, 4)],  # No gamma values
            undirected_edges=[(5, 3), (5, 6), (6, 7), (4, 8), (8, 1), (8, 2)],
            nodes=[
                (3, {"label": "C"}),
                (7, {"label": "D"}),
...
class TestInducedSplits:
    """Test induced_splits function for SemiDirectedPhyNetwork."""

    def test_simple_tree(self) -> None:
        """Test induced_splits on a simple tree."""
        # Use a valid tree structure (internal node must have degree >= 3)
        net = SemiDirectedPhyNetwork(
    ...
    def test_network_with_hybrid(self) -> None:
        ...
        splits = induced_splits(LEVEL_1_SDNETWORK_SINGLE_HYBRID)
        assert len(splits) >= 1
        # Check that all splits cover all taxa
        all_taxa = set(LEVEL_1_SDNETWORK_SINGLE_HYBRID.taxa)
        for split in splits:
    ...
    def test_single_taxon_network(self) -> None:
        ...
        # Single-taxon network: just a leaf node
        net = SemiDirectedPhyNetwork(nodes=[(1, {"label": "A"})])
        splits = induced_splits(net)
    ...
    def test_tree_of_blobs_same_splits(self) -> None:
        ...
        original_splits = induced_splits(LEVEL_1_SDNETWORK_SINGLE_HYBRID)
        blob_tree = tree_of_blobs(LEVEL_1_SDNETWORK_SINGLE_HYBRID)
    ...
    def test_tree_of_blobs_same_splits_multiple_blobs_level2(self) -> None:
        ...
        from tests.fixtures.sd_networks import LEVEL_2_SDNETWORK_MULTIPLE_BLOBS

        original_splits = induced_splits(LEVEL_2_SDNETWORK_MULTIPLE_BLOBS)
        blob_tree = tree_of_blobs(LEVEL_2_SDNETWORK_MULTIPLE_BLOBS)
        blob_tree_splits = induced_splits(blob_tree)
...
class TestSplitFromCutedge:
    """Test split_from_cutedge function for SemiDirectedPhyNetwork."""
    ...
    def test_directed_edge_cutedge(self) -> None:
        """Test split_from_cutedge on a directed edge (hybrid edge)."""
    ...
    def test_non_cutedge(self) -> None:
        ...
        cut_edges_set = cut_edges(net)
        all_edges = set()
        for u, v, key in net._graph.directed_edges_iter(keys=True):
            all_edges.add((u, v, key))
        for u, v, key in net._graph.undirected_edges_iter(keys=True):
            all_edges.add((u, v, key))
    ...
    def test_multiple_parallel_edges_no_key(self) -> None:
        """Test split_from_cutedge raises error when multiple parallel edges exist and key is None."""
        # Note: SemiDirectedPhyNetwork doesn't allow parallel edges in the same way,
        # but we test the error handling
        net = SemiDirectedPhyNetwork(
            undirected_edges=[(3, 1), (3, 2), (3, 4)],
            nodes=[(1, {"label": "A"}), (2, {"label": "B"}), (4, {"label": "C"})],
        )
        # This should work fine with a single edge
        split = split_from_cutedge(net, 3, 1)
        assert isinstance(split, Split)
    ...
    def test_split_set1_on_u_side(self) -> None:
        ...
        assert "A" in split.set1 or "A" in split.set2
        # B and C should be on the side of node 3
        assert "B" in split.set1 or "B" in split.set2
        assert "C" in split.set1 or "C" in split.set2
        # A should be in a different set from B and C
        assert ("A" in split.set1 and {"B", "C"}.issubset(split.set2)) or (
            "A" in split.set2 and {"B", "C"}.issubset(split.set1)
        )
...
class TestToDNetwork:
    ...
    def test_branch_length_preserved(self) -> None:
        ...
        net = SemiDirectedPhyNetwork(
            undirected_edges=[{"u": 3, "v": 1, "key": 0, "branch_length": 1.5}, (3, 2), (3, 4)],
            nodes=[(1, {"label": "A"}), (2, {"label": "B"}), (4, {"label": "C"})],
        )
        d_net = to_d_network(net, root_location=(3, 1, 0))
...
class TestDisplayedQuartets:
    ...
    def test_network_more_than_four_taxa(self) -> None:
        ...
        net = SemiDirectedPhyNetwork(
            undirected_edges=[(7, 1), (7, 2), (7, 8), (8, 3), (8, 4), (8, 5), (8, 6)],
            nodes=[
                (1, {"label": "A"}),
                (2, {"label": "B"}),
                (3, {"label": "C"}),
...
class TestDistancesBlobDecompositionSD:
    """`distances` enumerates each blob's switchings separately; results must not change.

    The reference is the definition itself: aggregate over every global switching.
    The networks are built with reticulations in *different* blobs, which is exactly
    the case the decomposition changes.
    """
    ...
    def _brute_force(network, taxa, mode):
        ...
        for graph in _switchings(network, probability=(mode == "average")):
            matrix = TestDistancesBlobDecompositionSD._switching_matrix_nx(graph, taxa, network)
            if mode == "shortest":
                result = np.minimum(result, matrix)
            elif mode == "longest":
                result = np.maximum(result, matrix)
            else:
                weight = graph._directed.graph.get("probability", 1.0) or 1.0
    ...
    @staticmethod
    def _two_blob_network():
        """Two level-1 blobs, obtained by semi-directing a two-blob rooted network."""
        from phylozoo.core.network.dnetwork import DirectedPhyNetwork
        from phylozoo.core.network.dnetwork.derivations import to_sd_network

        labels = ["A", "B", "C", "D", "E", "F"]
        directed = DirectedPhyNetwork(
            edges=[
                ("rho", "a0"),
                ("rho", "b0"),
                ("a0", "a1"),
            ...
            nodes=[(name, {"label": name}) for name in labels],
        )
        with no_validation():
            return to_sd_network(directed)

    def test_network_really_has_two_blobs(self):
        """Guard the premise: if this became single-blob the tests below would be vacuous."""
        from phylozoo.core.network.sdnetwork.derivations import _hybrid_blob_groups

        groups = _hybrid_blob_groups(self._two_blob_network())
        assert len(groups) == 2
...
class TestDistancesWithParallelEdgesSD:
    ...
    def test_network_really_has_parallel_edges(self):
        ...
        assert {edge[0] for edge in parents} == {"u"}
...
class TestDisplayedSplitsBlobDecompositionSD:
...
class TestSwitchingMachinerySD:
    """The pieces shared by `distances` and `displayed_splits`, semi-directed side."""

    @staticmethod
    def _network():
        return TestDistancesBlobDecompositionSD._two_blob_network()

    def test_hybrid_parent_edges_lists_every_parent_edge(self):
        from phylozoo.core.network.sdnetwork.derivations import _hybrid_parent_edges

        network = self._network()
        parents = _hybrid_parent_edges(network)
        assert set(parents) == set(network.hybrid_nodes)
        assert all(len(edges) == 2 for edges in parents.values())

    def test_plan_groups_reference_and_masses(self):
        from phylozoo.core.network.sdnetwork.derivations import _BlobSwitchings
        ...
        for index in range(len(plan.groups)):
            assert len(list(plan.local_switchings(index))) == 2
    ...
    def test_branch_lengths_both_orientations_and_default(self):
        from phylozoo.core.network.sdnetwork.derivations import _branch_lengths
