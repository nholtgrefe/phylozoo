...
class MSA(IOMixin):
    ...
    def __init__(
        ...
        if len(set(lengths)) != 1:
            raise PhyloZooValueError(
                f"All sequences must have the same length. " f"Found lengths: {set(lengths)}"
            )
    ...
    def from_coded_array(
        ...
        if not isinstance(taxa_order, tuple):
            raise PhyloZooTypeError(f"taxa_order must be a tuple, got {type(taxa_order).__name__}")
        ...
        if coded_array.dtype != np.int8:
            raise PhyloZooValueError(f"coded_array must have dtype int8, got {coded_array.dtype}")
        ...
        msa._taxa_to_index = {taxon: idx for idx, taxon in enumerate(taxa_order)}
    ...
    def _build_coded_array(self, sequences: dict[str, str]) -> np.ndarray:
        ...
        for taxon in self._taxa_order:
            ...
            coded = self._lookup_table[np.frombuffer(seq.encode("ascii"), dtype=np.uint8)]
    ...
    def __repr__(self) -> str:
        ...
        """
        return f"MSA(num_taxa={self._num_taxa}, " f"sequence_length={self._sequence_length})"
