...
def identify_vertices(
    graph: "MixedMultiGraph", vertices: list[T], merged_attrs: dict[str, Any] | None = None
) -> None:
    """
    ...
    edges_to_create: dict[tuple[T, T], set[str]] = (
        {}
    )  # (u, v) -> set of edge types ('directed' or 'undirected')
    ...
    if merged_attrs is not None:
        ...
        for attr_name in attrs_to_remove:
            if (
                first_vertex in graph._undirected.nodes()
                and attr_name in graph._undirected.nodes[first_vertex]
            ):
                del graph._undirected.nodes[first_vertex][attr_name]
            if (
                first_vertex in graph._directed.nodes()
...
def suppress_degree2_node(
    graph: "MixedMultiGraph", node: T, merged_attrs: dict[str, Any] | None = None
) -> None:
    """
    ...
    if type1 == "undirected" and type2 == "undirected":
        ...
        raise PhyloZooValueError(
            f"Unexpected edge type combination for node {node}: " f"{type1} and {type2}"
        )


def identify_parallel_edge(
    graph: "MixedMultiGraph", u: T, v: T, merged_attrs: dict[str, Any] | None = None
) -> None:
    """
