...
def quartet_distance(
    ...
    if rho_a > rho_o or rho_c > rho_s:
        raise PhyloZooValueError("Rho vector must satisfy: rho_a <= rho_o and rho_c <= rho_s")
...
def _rho_distance(
    ...
    if num_quartets == 1:
        ...
        if circular_orderings is None or len(circular_orderings) == 0:
            raise PhyloZooValueError("Profile with 2 quartets must have a circular ordering")
        ...
        raise PhyloZooValueError(f"Profile must have 1 or 2 quartets, got {num_quartets}")
