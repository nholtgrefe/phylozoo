...
def dnetwork_from_dmgraph(graph: DirectedMultiGraph[T]) -> DirectedPhyNetwork[T]:
    """
    Create a DirectedPhyNetwork from a DirectedMultiGraph.

    Extracts edges and node labels from the graph to create a new network.
    Node labels are extracted from the 'label' node attribute.
    ...
    >>> dmgraph = DirectedMultiGraph(edges=[(1, 2), (2, 3)])
    >>> dmgraph.add_node(3, label='A')
    >>> net = dnetwork_from_dmgraph(dmgraph)
    >>> net.get_label(3)
    'A'
    """
    # Extract edges
